	...
	switch (href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("prefsquad")
					var/new_pref_squad = tgui_input_checkboxes(user, "Choose your preferred squads in order of priority or none for 'No Preference'.", "Squad Preference", list("Alpha", "Bravo", "Tango", "Delta", "Oscar", "Kilo"), min_checked=6)
